...
// ========================
// SUB-SERVICE INFO EXPANSION - MODAL APPROACH
// ========================

document.addEventListener('DOMContentLoaded', () => {
  
  // Initialize info modal for Editorial Stage
  initializeSubServiceModal();
  
  function initializeSubServiceModal() {
    // Get all sub-service items in Editorial Stage
    const editorialCard = document.querySelector('[data-parent="editorial"]');
    if (!editorialCard) return;
    
    const subServiceItems = editorialCard.querySelectorAll('.sub-service-item');
    
    // Create modal container (only once)
    let modal = document.getElementById('sub-service-modal');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'sub-service-modal';
      modal.className = 'sub-service-modal';
      modal.innerHTML = `
        <div class="sub-service-modal-overlay"></div>
        <div class="sub-service-modal-content">
          <button class="sub-service-modal-close" aria-label="Close">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="18" y1="6" x2="6" y2="18"></line>
              <line x1="6" y1="6" x2="18" y2="18"></line>
            </svg>
          </button>
          <div class="sub-service-modal-body">
    ...
    const contentMap = {
      0: {
        ...
        description: "Focuses on the big picture of one's manuscript. Here, a writer works on structure, plot, character development, pacing, and overall narrative flow to ensure that the narrative achieves its full potential."
      },
      1: {
        title: 'Copyediting',
      ...
      2: {
        title: 'Proofreading',
        description: 'The final quality check before finalising the manuscript for its publication.Here, a proofreader catches any remaining typos, formatting inconsistencies, and minor errors to ensure that the manuscript is publication-ready.'
      }
    };
    ...
    subServiceItems.forEach((item, index) => {
      ...
      infoButton.classList.add('info-toggle-btn');
      infoButton.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>';
      infoButton.setAttribute('aria-label', 'View information');
      
      // Insert button into item
      ...
      infoButton.addEventListener('click', (e) => {
        ...
        const content = contentMap[index] || { 
          ...
          description: 'Detailed information about this service will be added here.' 
        };
    ...
    const closeBtn = modal.querySelector('.sub-service-modal-close');
    const overlay = modal.querySelector('.sub-service-modal-overlay');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeDesignModal() {
    const designCard = document.querySelector('[data-parent="design"]');
    if (!designCard) return;
    
    const subServiceItems = designCard.querySelectorAll('.sub-service-item');
    const modal = document.getElementById('sub-service-modal');
    if (!modal) return;
    
    const contentMap = {
      0: {
        title: 'Book Layout and Typesetting',
        description: "This is where arrangement of finalized manuscript's text, images, and design elements on each page happens. This stage ensures consistent fonts, spacing, margins, and alignment so the book is easy to read and visually appealing. In short, it’s about turning one's manuscript into a clean, reader-ready publication.."
      },
      1: {
        title: 'Cover Design',
        description: "Cover design involves creating the complete visual exterior of a book that includes its front, spine, and back cover. It blends imagery, color, typography, and layout to reflect the book’s tone and genre while maintaining visual harmony. The design also incorporates back-cover elements such as the blurb, author bio, and barcode, ensuring a polished and professional presentation."
      },
      2: {
        title: 'Illustrations & Graphics',
        description: 'Illustrations and graphics add visual appeal and clarity, helping readers connect with the content more deeply.'
      }
    };
    ...
    subServiceItems.forEach((item, index) => {
      ...
      infoButton.addEventListener('click', (e) => {
        ...
        const content = contentMap[index] || { 
          title: 'Service Information', 
          description: 'Detailed information will be added here.' 
        };
...
// ====================================
// PUBLISHING AND DISTRIBUTION MODAL
// ====================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeDesignModal() {
    const designCard = document.querySelector('[data-parent="publishing"]');
    if (!designCard) return;
    ...
    const contentMap = {
      0: {
        ...
        description: "The stage where a unique ISBN is registered, formally establishing the book’s identity in publishing databases and marketplaces. This is essential for distribution and sales tracking."
      },
      1: {
        title: 'Printing',
        description: 'Printing marks the moment the book takes tangible form — each page, color, and finish carefully produced to match the approved design. This stage ensures the physical quality of the book meets industry standards and author expectations.'
      },
      2: {
        title: 'E-book Conversion',
        description: 'E-book conversion prepares the book for digital distribution by formatting it for e-readers and online platforms, while ensuring it retains design quality and reads beautifully on screens of all sizes.'
      },
      3: {
        title: 'Distribution',
        description: 'Distribution handles the logistics of making the book widely available, including coordinating print copies, digital uploads, and placement in sales channels.'
      }
    };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeDesignModal() {
    const designCard = document.querySelector('[data-parent="marketing"]');
    if (!designCard) return;
    ...
    const contentMap = {
      0: {
        ...
        description: 'Develop a unique brand identity for your author persona, including logo design and branding guidelines.'
      },
      
      1: {
        title: 'Book Launch Campaigns',
        description: 'Strategic marketing campaigns to promote your book launch, including social media promotion, email marketing, and influencer outreach.'
      },
      2: {
        title: 'Digital Marketing',
        description: 'Promote your book through targeted online advertising, social media campaigns, and email marketing strategies.'
      },
      3: {
        title: 'PR & Media Outreach',
        description: 'Comprehensive outreach services to connect your book with media outlets, bloggers, and influencers for maximum visibility.'
      },
      4: {
        title: 'Event and Community Engagement',
        description: 'Comprehensive outreach services to connect your book with media outlets, bloggers, and influencers for maximum visibility.'
      },
      5: {
        title: 'Content Marketing and SEO',
        description: 'Optimize your online presence through effective content marketing strategies and search engine optimization techniques.'
      }
...
// ========================
// LEGAL AND ADMINISTRATIVE MODAL
// ========================
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeDesignModal() {
    const designCard = document.querySelector('[data-parent="legal"]');
    if (!designCard) return;
    ...
    const contentMap = {
      0: {
        title: 'Copyright and ISBN',
        description: 'Ensure your book is protected with copyright and properly registered with an ISBN.'
      },
      
      1: {
        title: 'Contracts and Agreements',
        description: 'Draft and review contracts and agreements to protect your rights and interests.'
      },
      2: {
        title: 'Rights Management',
        description: 'Ensure your book rights are protected and managed effectively.'
      },
      3: {
        title: 'Permission and Licensing',
        description: 'Ensure your book is properly licensed and permissions are secured for all content.'
      },
      4: {
        title: 'Administrative Essentials',
        description: 'Comprehensive administrative support to ensure your publishing process runs smoothly.'
      },
      5: {
        title: 'Author Protection',
        description: 'Ensure your rights as an author are protected.'
      }
